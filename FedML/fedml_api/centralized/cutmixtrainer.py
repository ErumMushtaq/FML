    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            for i_batch, sampled_batch in enumerate(self.train_global):
                ...
                aug_batch = maybe_to_torch(sampled_batch[3]) #Z's
                aug_batch.type(torch.FloatTensor)

                mask_ = maybe_to_torch(sampled_batch[2])

                volume_batch = volume_batch.to(self.device)
                label_batch = label_batch.to(self.device)
                aug_batch = aug_batch.to(self.device, dtype=torch.float)
                # aug_batch.type(torch.FloatTensor)

                unlabeled_volume_batch = volume_batch[1:]
                aug_unlabeled_volume_batch =aug_batch[1:]
                labeled_volume_batch = volume_batch[:1]

                # ICT mix factors
                ict_mix_factors = np.random.beta(
                    self.args.ict_alpha, self.args.ict_alpha, size=(2//2, 1, 1, 1, 1))
                ...
                aug_unlabeled_volume_batch_0 = aug_unlabeled_volume_batch[0:1, ...]
                aug_unlabeled_volume_batch_1 = aug_unlabeled_volume_batch[1:, ...]
                # logging.info(aug_unlabeled_volume_batch_0.shape)
                # exit()
                # Mix images
                batch_ux_mixed = unlabeled_volume_batch_0 * \
                (1.0 - ict_mix_factors) + \
                unlabeled_volume_batch_1 * ict_mix_factors #x'

                aug_batch_ux_mixed = unlabeled_volume_batch_0 * \
                (1.0 - ict_mix_factors) + \
                unlabeled_volume_batch_1 * ict_mix_factors #z'

                input_volume_batch = torch.cat([labeled_volume_batch, batch_ux_mixed, aug_batch_ux_mixed], dim=0)
                ...
                with torch.no_grad():
                    ...
                    aug_ema_output_ux0 = torch.softmax(
                        self.ema_model(aug_unlabeled_volume_batch_0), dim=1)
                    # exit()
                    added_ux0_aug_ux0 = (ema_output_ux0 + aug_ema_output_ux0)/2
                    del ema_output_ux0, aug_ema_output_ux0
                    ema_output_ux1 = torch.softmax(
                        self.ema_model(unlabeled_volume_batch_1), dim=1)
                    aug_ema_output_ux1 = torch.softmax(
                        self.ema_model(aug_unlabeled_volume_batch_1), dim=1)
                    added_ux1_aug_ux1 = (ema_output_ux1 + aug_ema_output_ux1)/2
                    del ema_output_ux1, aug_ema_output_ux1
                    # exit()
                    # aug_ema_output_ux1 = torch.softmax(
                    #     self.ema_model(aug_unlabeled_volume_batch_1), dim=1)
                    ...
                    batch_pred_mixed = added_ux0_aug_ux0 * \
                        (1.0 - ict_mix_factors) + added_ux1_aug_ux1 * ict_mix_factors
                ...
                unsup_loss_ce = self.ce_loss(outputs[1:2],
                              batch_pred_mixed_one_encod.long())
                consistency_loss = self.dice_loss(outputs_soft[1:2], batch_pred_mixed_one_encod.unsqueeze(1)) + unsup_loss_ce
                ...
                # # add uncertainty
                # var_l, exp_var_l = cal_variance(outputs_soft[1:2], outputs_soft[2:3])
                # cps_loss = exp_var_l * consistency_loss
                # loss = supervised_loss + consistency_weight * (cps_loss)
