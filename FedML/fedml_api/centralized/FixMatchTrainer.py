...
from fed_kits19.loss import get_current_consistency_weight, update_ema_variables, DiceLoss, BaselineLoss
import random
import shutil
try:
...
class FixMatchTrainer(object):
    ...
    def __init__(self, student_model, teacher_model, dataset, device, args):
        ...
        # loss function
        self.loss = BaselineLoss({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False}, {})
    ...
    def train(self):
        logging.info(' Train Function Called ')
        self.model.to(self.device)
        ...
        for epoch in range(self.args.epochs):
            ...
            for i_batch, sampled_batch in enumerate(self.train_global):
                ...
                # logging.info('jfehwig')

                unlabeled_volume_batch = volume_batch[1:]
                labeled_volume_batch = volume_batch[:1]

                # # testing
                # outputs = self.model(labeled_volume_batch)
                # outputs_soft = torch.softmax(outputs, dim=1)
                # loss_ce = self.ce_loss(outputs[:2],
                #                label_batch[:2][:].squeeze(1).long())
                # loss_dice = self.dice_loss(outputs_soft[:2], label_batch[:2])
                # supervised_loss = 0.5 * (loss_dice + loss_ce)
                # loss = supervised_loss
                # unsupervised_loss = supervised_loss
                # consistency_weight = 0.0

                str_unlabeled_volume_batch = self.strong_tr_transform(unlabeled_volume_batch)
                weak_unlabeled_volume_batch = self.weak_tr_transform(unlabeled_volume_batch)
                input_data = torch.cat([labeled_volume_batch, weak_unlabeled_volume_batch, str_unlabeled_volume_batch], dim=0)
                outputs = self.model(input_data)
                outputs_soft = torch.softmax(outputs, dim=1)
                # logging.info(label_batch[:1].shape)
                # supervised_loss = self.loss(outputs_soft[:1],label_batch[:1])
                loss_ce = self.ce_loss(outputs[:1],
                              label_batch[:1][:].squeeze(1).long())
                outputs_soft_u_w, outputs_soft_u_s = outputs_soft[1:].chunk(2)
                loss_dice = self.dice_loss(outputs_soft[:1], label_batch[:1])
                supervised_loss = 0.5 * (loss_dice + loss_ce)
                # del outputs
                # logging.info(outputs_soft_u_w.shape)
                ...
                # logging.info(batch_one_encod.unsqueeze(1).shape)
                # logging.info(outputs_soft[1:].shape)
                # torch.masked_select(x, mask)
                # unsupervised_loss = torch.mean((outputs_soft_u_s - batch_one_encod) ** 2)
                unsupervised_loss  = self.dice_loss(outputs_soft_u_s, batch_one_encod, mask = mask)
                # unsupervised_loss = self.loss(outputs_soft_u_s, batch_one_encod, loss_mask = mask)
                ...
                if self.args.is_debug == 1 and iteration_count == 10:
                    break
                wandb.log({" consistency weight": consistency_weight, " iteration_count ": iteration_count})
                # print('One iteration run   ')

                # del input_data, volume_batch, label_batch, loss, str_unlabeled_volume_batch, weak_unlabeled_volume_batch, outputs_soft_u_w, outputs_soft_u_s
    ...
    def test(self, test_data, device, args, epoch):
        ...
        with torch.no_grad():
            for (X, y) in test_data:
                ...
                if args.is_debug == 1 and iteration_count == 20:
                    break
