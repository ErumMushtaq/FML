...
class FedAvgTrainer_(ModelTrainer):
    ...
    def set_model_params(self, model_parameters):
        ...
        self.prediction_model.load_state_dict(model_parameters)
            ...
            for i_batch, sampled_batch in enumerate(train_data):
                ...
                with torch.no_grad():
                    ema_output_ux0 = torch.softmax(
                        self.prediction_model(unlabeled_volume_batch_0), dim=1)
                    ema_output_ux1 = torch.softmax(
                        self.prediction_model(unlabeled_volume_batch_1), dim=1)
