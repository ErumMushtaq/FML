...
class FedAVGServerManager(ServerManager):
    ...
    def handle_message_receive_model_from_client(self, msg_params):
        ...
        train_loss = msg_params.get(MyMessage.MSG_ARG_KEY_TRAIN_LOSS)

        self.aggregator.add_local_trained_result(sender_id - 1, model_params, local_sample_number, train_loss, self.round_idx)
        b_all_received = self.aggregator.check_whether_all_receive()
        logging.info("b_all_received = " + str(b_all_received))
        # if self.args.weight_scheme == 'dr_weight':
