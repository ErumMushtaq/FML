...
class FedAVGAggregator(object):
    ...
    def add_local_trained_result(self, index, model_params, sample_num, train_loss, round_idx):
        ...
        self.sample_num_dict[index] = sample_num * train_loss['weight'] # In this case there is no weighting effectively in unsupervised
        # self.sample_num_dict[index] = sample_num 

        """if index in range(2, 6):
            self.weight_[index] = train_loss['weight'] #below part is the term we are using weighting
    ...
    """def client_sampling(self, round_idx, client_num_in_total, client_num_per_round):#hardcoded right now assuming 0 and 1 have labelled data
        if round_idx % 10 > 5:
            client_indexes = [2,3,4,5]
            client_indexes = random.sample(client_indexes,2)
        else:
            client_indexes = [0, 1]
        logging.info("client num per round = %s" % str(client_num_per_round))
    ...
    def client_sampling(self, round_idx, client_num_in_total, client_num_per_round):
        if client_num_in_total == client_num_per_round:
            ...
            if round_idx % 10 < 5: # select all supervised silos
                logging.info("Supervised training Round")
                client_indexes = [client_index for client_index in {4, 5, 6, 12, 8, 9}]
            else: # select unsupervised silos
                np.random.seed(round_idx) 
                client_indexes = np.random.choice([0,1,2,3,7,8,9,10,11], min(client_num_per_round, client_num_in_total), replace=False)
                # client_indexes = [client_index for client_index in {0,1,2,3,7,8,9,10,11}]
    ...
    def test_on_server_for_all_clients(self, round_idx):
        ...
        #  self, train_data_local_dict, test_data_local_dict, device, args=None, round_idx=None, mean_train_loss = 0.0
        if self.trainer.test_on_the_server(self.train_data_local_dict, self.test_global, self.device, self.args, round_idx, mean_train_loss):
            return
