...
class FedAvgTrainer_(ModelTrainer):
    ...
    def unsuptrain(self, train_data, device, args, client_idx, N, round_, Client_lu_ID):
        ...
        for epoch in range(self.args.epochs):
            ...
            for i_batch, sampled_batch in enumerate(train_data):
                ...
                # pseudo labels of the entities that satisfy the threhold
                pseudo_labels = torch.masked_select(batch_one_encod, (mask==1))
                label_batch = torch.masked_select(label_batch, (mask==1))
                ...
                del pseudo_labels, label_batch, volume_batch, loss, str_unlabeled_volume_batch, weak_unlabeled_volume_batch, mask
    ...
    def suptrain(self, train_data, device, args, client_idx, N, round_, Client_lu_ID):
        ...
        for epoch in range(self.args.epochs):
            ...
            self.train_loss = running_loss / iteration_count
            self.training_loss_list.append(self.train_loss)
        dict = {'train_loss': self.train_loss, 'weight': 1, 'pseudo_label_kidney': 1.0, 'pseduo_label_tumor': 1.0}
        # return train_loss
        return dict
