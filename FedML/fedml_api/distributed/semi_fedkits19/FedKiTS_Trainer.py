...
class FedAvgTrainer_(ModelTrainer):
    def __init__(self, model, loss, ltr, utr, vtr, args=None):
        self.model = model
        self.args = args
    ...
    def semitrain2(self, train_data, device, args, client_idx, N, round, Client_lu_ID):
        if self.args.apply_threshold  == 'True' and self.args.phase == 'student_training':
            self.loss = DC_and_CE_loss({'batch_dice': True, 'smooth': 1e-5, 'do_bg': False}, {}, ignore_label=255)
        else:
            self.loss = DC_and_CE_loss({'batch_dice': True, 'smooth': 1e-5, 'do_bg': False}, {})
    ...
    def semitrain(self, train_data, device, args, client_idx, N, round, Client_lu_ID):
        if self.args.apply_threshold == 'True' and self.args.phase == 'student_training':
            self.loss = DC_and_CE_loss({'batch_dice': True, 'smooth': 1e-5, 'do_bg': False}, {}, ignore_label=255)
        else:
            self.loss = DC_and_CE_loss({'batch_dice': True, 'smooth': 1e-5, 'do_bg': False}, {})
        ...
        for epoch in range(args.epochs):
            ...
            while iteration_count < 6:                
                for sample in train_data:
                    ...
                    # sample[1] = maybe_to_torch(sample[1])
                    # print(sample[0].shape)
                    number = np.random.binomial(1, 0.5)
                    if Client_lu_ID == 1 or number == 1:
                        case_identifier = sample[2]
                        data_dictt = {'data': sample[0].numpy(), 'seg': sample[1].numpy()}
                        data_dict = self.weak_tr(**data_dictt)
                        ...
                        if args.is_debug == 1 and iteration_count == 2:
                            break
                        if Client_lu_ID == 0 and iteration_count == 6:
                            break
                self.train_loss = running_loss / iteration_count
                self.training_loss_list.append(self.train_loss)
    ...
    # def run_iteration(self, data_generator, round, device = None, do_backprop=True,  client_lu_ID =0, run_online_evaluation=False, client_idx = 0):
    ...
    #     self.seg_shape = target.shape
    #     if client_lu_ID == 1: #unlabelled
