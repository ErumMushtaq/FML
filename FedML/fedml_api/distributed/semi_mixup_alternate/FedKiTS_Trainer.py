...
class FedAvgTrainer_(ModelTrainer):
    ...
    def unsuptrain(self, train_data, device, args, client_idx, N, round_, Client_lu_ID):
        if round_ % 10 < 5:
            self.train_loss = 0.0
            dict = {'train_loss': self.train_loss, 'weight': 0.0, 'pseudo_label_kidney': 0.0, 'pseduo_label_tumor': 0.0}
        ...
        for epoch in range(self.args.epochs):
            ...
            for i_batch, sampled_batch in enumerate(train_data):
                ...
                with torch.no_grad():
                    ...
                    pseudo_label0 = torch.argmax(ema_output_ux0, dim=1)
                    ema_output_ux1 = torch.softmax(
                        self.ema_model(unlabeled_volume_batch_1), dim=1)
                    pseudo_label1 = torch.argmax(ema_output_ux1, dim=1)
                    pseudo_labels = torch.cat([pseudo_label0, pseudo_label1], dim=0)
                ...
                # logging.info(pseudo_labels.shape)
                # logging.info(label_batch.shape)
                
                pseudo_labels = pseudo_labels.unsqueeze(1).detach().cpu()
                label_batch = label_batch.detach().cpu()
                dice1, dice2, dice3 = self.evaluation(pseudo_labels.unsqueeze(1), label_batch)
                if iteration_count % self.args.frequency_of_the_test == 0:
                    logging.info("iterarion Count = %d, Acc = %f," % (iteration_count, ((dice2 + dice3) / 2)))
                kidney_pred.append(dice2)
                tumor_pred.append(dice3)
        ...
        # wandb.log({" lr ": self.optimizer.param_groups[0]['lr'], "round ": round})
            ...
            kidney_dice_score = np.mean(kidney_pred)
            tumor_dice_score = np.mean(tumor_pred)
            self.train_loss = running_loss / iteration_count
            self.training_loss_list.append(self.train_loss)
        dict = {'train_loss': self.train_loss, 'weight': consistency_weight, 'pseudo_label_kidney': kidney_dice_score, 'pseduo_label_tumor': tumor_dice_score}
        # return train_loss
        return dict
    ...
    def suptrain(self, train_data, device, args, client_idx, N, round_, Client_lu_ID):
        if round_ % 10 >= 5:
            self.train_loss = 0.0
            dict = {'train_loss': self.train_loss, 'weight': 0.0, 'pseudo_label_kidney': 1.0, 'pseduo_label_tumor': 1.0}
        ...
        for epoch in range(self.args.epochs):
            ...
            self.train_loss = running_loss / iteration_count
            self.training_loss_list.append(self.train_loss)
        dict = {'train_loss': self.train_loss, 'weight': 1.0,  'pseudo_label_kidney': 1.0, 'pseduo_label_tumor': 1.0}
        # return train_loss
        return dict
