...
class FedAvgTrainer_(ModelTrainer):
    ...
    def unsuptrain(self, train_data, device, args, client_idx, N, round_, Client_lu_ID):
        ...
        for epoch in range(self.args.epochs):
            ...
            for i_batch, sampled_batch in enumerate(train_data):
                ...
                aug_batch = maybe_to_torch(sampled_batch[3])

                volume_batch = volume_batch.to(device)
                label_batch = label_batch.to(device)
                aug_batch = aug_batch.to(device, dtype=torch.float)
                ...
                aug_unlabeled_volume_batch_0 = volume_batch[0:2, ...]
                aug_unlabeled_volume_batch_1 = volume_batch[2:, ...]
                ...
                input_volume_batch = torch.cat([batch_ux_mixed, aug_batch_ux_mixed], dim=0)
                ...
                with torch.no_grad():
                    ...
                    predictions_ = torch.cat([ema_output_ux0, ema_output_ux1], dim=0)
                    pseudo_labels = torch.argmax(predictions_, dim=1)


                # #l2 loss
                consistency_weight = get_current_consistency_weight(round_, self.args.consistency, self.args.consistency_rampup)
                ...
                unsup_loss_ce = self.ce_loss(outputs[0:2],
                              batch_pred_mixed_one_encod.long())
                
                # print(outputs_soft[:1].shape)
                # print(batch_pred_mixed_one_encod.unsqueeze(1).shape)
                consistency_loss = unsup_loss_ce + self.dice_loss(outputs_soft[0:2], batch_pred_mixed_one_encod.unsqueeze(1))
                loss =  consistency_loss
                ...
                dice1, dice2, dice3 = self.evaluation(pseudo_labels, label_batch)
        ...
        dict = {'train_loss': self.train_loss, 'weight': consistency_weight, 'Kidney_Dice':kidney_dice_score,'Tumor_Dice':tumor_dice_score}
        # return train_loss
        return dict
    ...
    def suptrain(self, train_data, device, args, client_idx, N, round_, Client_lu_ID):
        ...
        for epoch in range(self.args.epochs):
            ...
            self.train_loss = running_loss / iteration_count
            self.training_loss_list.append(self.train_loss)
        dict = {'train_loss': self.train_loss}
        # return train_loss
        return dict
    ...
    def test_on_the_server(
        ...
        logging.info(' Server Eval: Kidney Dice ' + str(score[0]) + ' Tumor Dice '+ str(score[1])+ ' Round '+str(round_idx))
