    ...
    def unsuptrain(self, train_data, device, args, client_idx, N, round_, Client_lu_ID):
        ...
        for epoch in range(self.args.epochs):
            ...
            for i_batch, sampled_batch in enumerate(train_data):
                ...
                # Fix Match
                str_unlabeled_volume_batch = self.strong_tr_transform(unlabeled_volume_batch_0)
                weak_unlabeled_volume_batch = unlabeled_volume_batch_0
                # weak_unlabeled_volume_batch = self.weak_tr_transform(unlabeled_volume_batch_0)
                ...
                input_data = torch.cat([batch_ux_mixed, weak_unlabeled_volume_batch, str_unlabeled_volume_batch], dim=0)
                ...
                # Fix Loss
                outputs_soft_u_w, outputs_soft_u_s = outputs_soft[2:].chunk(2)
                ...
                Fix_loss  = self.dice_loss(outputs_soft_u_s, batch_one_encod, mask = mask)
                # #l2 loss
                ...
                Mix_loss = self.dice_loss(outputs_soft[0:2], batch_pred_mixed_one_encod.unsqueeze(1))
                loss =  Mix_loss + Fix_loss
