...
class BoxMaskGenerator (MaskGenerator):
    ...
    def generate_params(self, n_masks, mask_shape, rng=None):
        ...
        if self.prop_by_area:
            ...
            # print(self.prop_range[0])
            # print(self.prop_range[1])
        ...
        # new_shape = (2, 2)
        if self.invert:
            masks = np.zeros((n_masks, 128) + mask_shape)
        else:
            masks = np.ones((n_masks, 128) + mask_shape)
        # print(masks.shape)
        # if self.invert:
        ...
        for i, sample_rectangles in enumerate(rectangles):
            for z in range(0,128):
                for y0, x0, y1, x1 in sample_rectangles:
                    masks[i, z, int(y0):int(y1), int(x0):int(x1)] = 1 - masks[i, z, int(y0):int(y1), int(x0):int(x1)]
        return masks
...
if __name__ == '__main__':
    ...
    boxmix_gen = BoxMaskGenerator(prop_range= cutmix_mask_prop_range, n_boxes= cutmix_boxmask_n_boxes,
    ...
    #     mask_generator
    # )
    params = boxmix_gen.generate_params(1, (2, 2))
    print(params)
    t_masks = boxmix_gen.torch_masks_from_params(params, (32, 32), 'cuda:0')
