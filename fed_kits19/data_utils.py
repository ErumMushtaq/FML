# from Enhancing Pseudo Label Quality for Semi-Supervised Domain-Generalized Medical Image Segmentation
...
def amp_spectrum_swap_3D( amp_local, amp_target, L=0.1 , ratio=0):
    
    a_local = np.fft.fftshift( amp_local, axes=(-3, -2, -1) )
    a_trg = np.fft.fftshift( amp_target, axes=(-3, -2, -1) )

    _,_, c, h, w = a_local.shape
    b = (  np.floor(np.amin((h,w))*L)  ).astype(int)
    c_h = np.floor(h/2.0).astype(int)
    c_w = np.floor(w/2.0).astype(int)
    ...
    a_local = np.fft.ifftshift( a_local, axes=(-3, -2, -1) )
    a_trg = np.fft.ifftshift( a_trg, axes=(-3, -2, -1))
    return a_local, a_trg



def freq_space_interpolation_3D(local_img, trg_img, L=0 , ratio=0):

    local_img_np = local_img
    tar_img_np = trg_img

    # get fft of local sample
    fft_local_np = np.fft.fft2( local_img_np, axes=(-3, -2, -1) )
    fft_trg_np = np.fft.fft2( tar_img_np, axes=(-3, -2, -1) )
    ...
    # swap the amplitude part of local image with target amplitude spectrum
    amp_local_, amp_trg_ = amp_spectrum_swap_3D( amp_local, amp_target, L=L , ratio=ratio)

    # get transformed image via inverse fft
    fft_local_ = amp_local_ * np.exp( 1j * pha_local )
    local_in_trg = np.fft.ifft2( fft_local_, axes=(-3, -2, -1) )
    local_in_trg = np.real(local_in_trg)

    fft_trg_ = amp_trg_ * np.exp( 1j * pha_trg )
    trg_in_local = np.fft.ifft2( fft_trg_, axes=(-3, -2, -1) )
    trg_in_local = np.real(trg_in_local)

    return local_in_trg, trg_in_local


def fourier_transform_3D(im_local, im_trg, L=0.01, i=1):
    # im_local = im_local.transpose((2, 0, 1))
    # im_trg = im_trg.transpose((2, 0, 1))
    org_im = np.squeeze(im_local)
    # save_image(np.squeeze(org_im[60,:,:]),'orginal.png')
    local_in_trg, trg_in_local = freq_space_interpolation_3D(im_local, im_trg, L=L, ratio=1-i)
    # local_in_trg = local_in_trg.transpose((1, 2, 0))
    # trg_in_local = trg_in_local.transpose((1, 2, 0))
    aug_image = np.squeeze(local_in_trg)
    # save_image(np.squeeze(aug_image[60,:,:]),'transform.png')
    # exit()
    return local_in_trg, trg_in_local
