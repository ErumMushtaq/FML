...
from FedML.fedml_api.distributed.semi_mixup_alternate.FedAvgAPI import FedML_FedAvg_distributed_alternate
from FedML.fedml_api.distributed.semi_mixup_alternate_weighted.FedAvgAPI import FedML_FedAvg_distributed_alternate_weighted

from FedML.fedml_api.distributed.semi_fixmatch.FedAvgAPI import FedML_FedAvg_distributed_fixmatch
from FedML.fedml_api.distributed.semi_mixup_proposed.FedAvgAPI import FedML_FedAvg_distributed_proposed
from FedML.fedml_api.distributed.semi_mixup_fourier.FedAvgAPI import FedML_FedAvg_distributed_mixup_fourier
...
from FedML.fedml_api.distributed.semi_mixup_alternate_fixed_pred.FedAvgAPI import FedML_FedAvg_distributed_alternate_fixed_pred
...
def init_training_device(process_ID, fl_worker_num, gpu_num_per_machine):
    ...
    if process_ID == 0:
        ...
        #device = torch.device("cuda:"+str(process_ID - 1) if torch.cuda.is_available() else "cpu")
        return device
    else:
...
if __name__ == "__main__":
    ...
    train_data_global = torch.utils.data.DataLoader(ltrain_dataset, batch_size=2, shuffle=True)

    utrain_dataset = SemiFedKiTS19(1, train=True, pooled=True, labelled=0)
    utrain_data_num = len(utrain_dataset)
    utrain_dataloader = torch.utils.data.DataLoader(utrain_dataset, batch_size=2, shuffle=True)
    ...
    if args.ssl_method == 'mixup':
        for i in range(args.client_number):
            if i <= 1:
                ...
                Client_ID_dict[i] = 1
                train_data_local_dict[i] = torch.utils.data.DataLoader(client_train_data, batch_size=2, shuffle=True, drop_last = True)
            else:
                client_train_data = SemiFedKiTS19(i, train=True, pooled=False, labelled=0)  # unlabelled
                Client_ID_dict[i] = 0
                train_data_local_dict[i] = torch.utils.data.DataLoader(client_train_data, batch_size=4, shuffle=True, drop_last = True)
    ...
    elif args.ssl_method == 'mixup_fourier':
        for i in range(args.client_number):
            if i <= 1:
                client_train_data = SemiFedKiTS19(i, train=True, pooled=False, labelled=1) #labelled
                Client_ID_dict[i] = 1
        ...
        model = create_model(args, model_name=args.model)
        ema_model = create_model(args, model_name=args.model, ema=True)
        FedML_FedAvg_distributed_mixup_fourier(
            process_id,
            worker_number,
            device,
            comm,
    ...
    elif args.ssl_method == 'mixup_alternate':
        for i in range(args.client_number):
            if i <= 1:
                client_train_data = SemiFedKiTS19(i, train=True, pooled=False, labelled=1) #labelled
                Client_ID_dict[i] = 1
        ...
        model = create_model(args, model_name=args.model)
        ema_model = create_model(args, model_name=args.model, ema=True)
        FedML_FedAvg_distributed_alternate(
            process_id,
            worker_number,
            device,
            comm,
    ...
    elif args.ssl_method == 'mixup_alternate_weighted':
        ...
        model = create_model(args, model_name=args.model)
        ema_model = create_model(args, model_name=args.model, ema=True)
        FedML_FedAvg_distributed_alternate_weighted(
            process_id,
            worker_number,
            device,
            comm,
    ...
    elif args.ssl_method == 'mixup_alternate_fixed_pred':
        for i in range(args.client_number):
            if i <= 1:
                client_train_data = SemiFedKiTS19(i, train=True, pooled=False, labelled=1) #labelled
                Client_ID_dict[i] = 1
        ...
        model = create_model(args, model_name=args.model)
        ema_model = create_model(args, model_name=args.model, ema=True)
        FedML_FedAvg_distributed_alternate_fixed_pred(
            process_id,
            worker_number,
            device,
            comm,
    ...
    elif args.ssl_method == 'fixmatch' or args.ssl_method == 'nvidia': # nvidia technique uses first 500 rounds for semi-supervised training.
        for i in range(args.client_number):
            if i <= 1:
                client_train_data = SemiFedKiTS19(i, train=True, pooled=False, labelled=1) #labelled
                Client_ID_dict[i] = 1 #sup
                train_data_local_dict[i] = torch.utils.data.DataLoader(client_train_data, batch_size=2, shuffle=True, num_workers=1, drop_last = True)
            else:
                client_train_data = SemiFedKiTS19(i, train=True, pooled=False, labelled=0)  # unlabelled
                Client_ID_dict[i] = 0 #unsup
                train_data_local_dict[i] = torch.utils.data.DataLoader(client_train_data, batch_size=2, shuffle=True, num_workers=1, drop_last = True)
        ...
        model = create_model(args, model_name=args.model)
        ema_model = create_model(args, model_name=args.model, ema=True)
        FedML_FedAvg_distributed_fixmatch(
            process_id,
            worker_number,
            device,
            comm,
        ...
        for i in range(args.client_number):
            if i <= 1:
                ...
                client_train_data = SemiFedKiTS19(i, train=True, pooled=False, labelled=0)  # unlabelled
                Client_ID_dict[i] = 0
                train_data_local_dict[i] = torch.utils.data.DataLoader(client_train_data, batch_size=4, shuffle=True, num_workers=1, drop_last = True)
            data_local_num_dict[i] = len(client_train_data)
        ...
        model = create_model(args, model_name=args.model)
        ema_model = create_model(args, model_name=args.model, ema=True)
        FedML_FedAvg_distributed_proposed(
            process_id,
            worker_number,
            device,
            comm,
