...
from nnunet.backup_experiment_planning.DatasetAnalyzer import DatasetAnalyzer
from nnunet.backup_experiment_planning.common_utils import split_4d_nifti
from nnunet.paths import nnUNet_raw_data, nnUNet_cropped_data, preprocessing_output_dir
from nnunet.backup_preprocessing.cropping import ImageCropper
...
# def create_lists_from_original_dataset(base_folder, json_file):
#     lists = []
...
def crop(data_files, seg_files, index, cropped_dir):
    num_threads = 1
    imgcrop = ImageCropper(num_threads, cropped_dir)
    data, seg = imgcrop.run_cropping(data_files, seg_files, index)
