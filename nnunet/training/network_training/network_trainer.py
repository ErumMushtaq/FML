...
class NetworkTrainer(object):
    ...
    def do_split(self):
        ...
        logging.info(' Training data Length '+ str(len(self.dataset_tr)))
        logging.info(' Validation data Length ' + str(len(self.dataset_val)))
        # logging.info(self.dataset_tr)
        # logging.info(self.dataset_val)
        # exit()
    ...
    def load_best_checkpoint(self, train=True):
        ...
        if isfile(join(self.output_folder, "model_best.model")):
            self.best_rounds_modelkpoint(join(self.output_folder, "model_best.model"), train=train)
        else:
    ...
    def run_training(self):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            train_loss_curr = np.mean(train_losses_epoch)
            self.all_tr_losses.append(np.mean(train_losses_epoch))
            self.print_to_log_file("train loss : %.4f" % self.all_tr_losses[-1])
            wandb.log({"Train/loss: Composite": train_loss_curr, "epoch ": self.epoch})
            with torch.no_grad():
                # validation with train=False
                self.network.eval()
                val_losses = []
                print(' Number of validation batches per epoch ')
                print(self.num_val_batches_per_epoch)
                for b in range(self.num_val_batches_per_epoch):
                    l = self.run_iteration(self.val_gen, False, True)
                    # print(' Epoch 0 for validation')
                    # break
                    val_losses.append(l)
                    # print(self.online_eval_foreground_dc)
                    # val_acc = np.mean(self.online_eval_foreground_dc, axis=0)
                    # print(val_acc)
                    # break
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        # print(data.shape)
        # summary(self.network, (1, 128, 128, 128))
        if self.fp16:
            with autocast():
