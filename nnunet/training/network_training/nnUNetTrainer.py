...
class nnUNetTrainer(NetworkTrainer):
    def __init__(self, plans_file, fold, train_data, valid_data, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
                 unpack_data=True, deterministic=True, fp16=False, device = None):
        """
        :param deterministic:
        ...
        if train_data is not None:
            self.tr_gen = train_data
            self.val_gen = valid_data

    # Add Fed ML Trainer Functionality we need
    ...
    def initialize_network(self, force_load_plans=False):
        """
        ...
        # print('number of input of Channels '+str(self.num_input_channels))
        # print('number of base number of features ' + str(self.base_num_features))
        # print('number of classes '+str(self.num_classes))
        # print('net_numpool '+str(net_numpool))
        # print(' self.conv per stage '+str(self.conv_per_stage))
        # print(' conv op '+str(conv_op))
        # print(' norm op '+str(norm_op))
        # print(' norm op kwargs '+str(norm_op_kwargs))
        # print(' dropout op '+str(dropout_op))
        # print(' dropout_op_kwargs '+str(dropout_op_kwargs))
        # print(' net non lin '+str(net_nonlin))
        # print(' net nonlin kwargs '+str(net_nonlin_kwargs))
        # print(' net_num_pool_op_kernel sizes '+str(self.net_num_pool_op_kernel_sizes))
        # print('net conv kernel sizes '+str(self.net_conv_kernel_sizes))
        # print(self.batch_size)
        # exit()

        # net_num_pool_op_kernel_sizes = [[2, 2, 2], [2, 2, 2], [2, 2, 2], [2, 2, 2], [2, 2, 2]]
        # net_conv_kernel_sizes = [[3, 3, 3], [3, 3, 3], [3, 3, 3], [3, 3, 3], [3, 3, 3], [3, 3, 3]]
        # self.network = Generic_UNet(1, 32, 3, 5,
        #                             2, 2, nn.Conv3d, nn.InstanceNorm3d, {'eps': 1e-5, 'affine': True}, nn.Dropout3d,
        #                             {'p': 0, 'inplace': True},
        #                             nn.LeakyReLU, {'negative_slope': 1e-2, 'inplace': True}, False, False, lambda x: x, InitWeights_He(1e-2),
        ...
        net_num_pool_op_kernel_sizes = [[1, 2, 2], [2, 2, 2], [2, 2, 2], [1, 2, 2], [1, 2, 2]]
        ...
        # # summary(self.network, data)
        # exit()

        # logging.info(' Network Initializedddd ')
        if torch.cuda.is_available():
            logging.info(self.device)
            self.network = self.network.to(self.device)
        #     logging.info(' Networkkk assigned to GPU ')
            # self.network.cuda()
        model = self.network
        # logging.info(' Network Returned ')
        return model
    ...
    def predict_preprocessed_data_return_seg_and_softmax(self, data: np.ndarray, do_mirroring: bool = True,
        ...
        ret = self.network.predict_3D(data, do_mirroring=do_mirroring, mirror_axes=mirror_axes,
                                      ...
                                      mixed_precision=mixed_precision, device = self.device)
    ...
    def finish_online_evaluation(self):
        ...
        wandb.log({"Kidney Acc": global_dc_per_class[0], "round ": self.epoch})
        wandb.log({" Tumor Acc": global_dc_per_class[1], "round ": self.epoch})
