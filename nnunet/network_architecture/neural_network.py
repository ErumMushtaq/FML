...
class SegmentationNetwork(NeuralNetwork):
    ...
    def predict_3D(self, x: np.ndarray, do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2),
                   ...
                   verbose: bool = True, mixed_precision: bool = True, device: str = 'cuda0') -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        with context():
            with torch.no_grad():
                # print(' self.conv_op ' + str(self.conv_op))
                # print('use_sliding_window '+ str(use_sliding_window))
                if self.conv_op == nn.Conv3d:
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
                                          ...
                                          verbose: bool, device: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for x in steps[0]:
            ...
            for y in steps[1]:
                ...
                for z in steps[2]:
                    lb_z = z
                    ub_z = z + patch_size[2]
                    if verbose: print('Patch prediction starting ')
                    predicted_patch = self._internal_maybe_mirror_and_pred_3D(
                        data[None, :, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z], mirror_axes, do_mirroring,
                        gaussian_importance_map)[0]
                    # if verbose: print('1 Patch predicted ')
                    if all_in_gpu:
