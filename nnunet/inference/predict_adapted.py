...
def predict_cases(model, trainer, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    for preprocessed in preprocessing:
        ...
        # trainer.best_rounds_modelkpoint_ram(params[0], False)
        ...
        # # for p in params[1:]:
        #     trainer.best_rounds_modelkpoint_ram(p, False)
        #     softmax += trainer.predict_preprocessed_data_return_seg_and_softmax(
        #         d, do_mirroring=do_tta, mirror_axes=trainer.data_aug_params['mirror_axes'], use_sliding_window=True,
        #         step_size=step_size, use_gaussian=True, all_in_gpu=all_in_gpu,
        #         mixed_precision=mixed_precision)[1]
...
def predict_cases_fastest(model, trainer, list_of_lists, output_filenames, folds, num_threads_preprocessing,
                          num_threads_nifti_save, segs_from_prev_stage=None, do_tta=True, mixed_precision=True,
    ...
    for preprocessed in preprocessing:
        ...
        # preallocate the output arrays
        # same dtype as the return value in predict_preprocessed_data_return_seg_and_softmax (saves time)
        all_softmax_outputs = np.zeros((1, trainer.num_classes, *d.shape[1:]), dtype=np.float16)
        all_seg_outputs = np.zeros((1, *d.shape[1:]), dtype=int)

        # all_softmax_outputs = np.zeros((len(params), trainer.num_classes, *d.shape[1:]), dtype=np.float16)
        # all_seg_outputs = np.zeros((len(params), *d.shape[1:]), dtype=int)
        print("predicting", output_filename)

        #add function to load parameters
        # trainer.best_rounds_modelkpoint_ram(p, False)
        res = trainer.predict_preprocessed_data_return_seg_and_softmax(d, do_mirroring=do_tta,
                                                                           mirror_axes=trainer.data_aug_params['mirror_axes'],
                                                                           use_sliding_window=True,
...
def predict_from_folder(model, trainer, plans_file, input_folder, output_folder, folds,
                        save_npz, num_threads_preprocessing, num_threads_nifti_save,
                        lowres_segmentations,
                        part_id, num_parts, tta, mixed_precision = True,
    ...
    expected_num_modalities = load_pickle(plans_file)['num_modalities']
    ...
    if mode == "normal":
        ...
        # print(list_of_lists[part_id::num_parts])
        return predict_cases(model, trainer, list_of_lists[part_id::num_parts], output_files[part_id::num_parts], folds,
        ...
        return predict_cases_fast(model, trainer, list_of_lists[part_id::num_parts], output_files[part_id::num_parts], folds,
